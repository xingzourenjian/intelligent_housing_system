...
// PB0 ADC12_IN8 MQ7传感器AO口
void MQ7_sensor_init(void)
{
    ADC_init(GPIO_Pin_0, ADC_Channel_8);
}
...
{
    return 1.0 * get_ADC_value(ADC_Channel_8) / 4095 * 3.3;
}

void show_MQ7_sensor_voltage_value_OLED(uint8_t line, uint8_t column)
{
    float value = get_MQ7_sensor_voltage_value();
